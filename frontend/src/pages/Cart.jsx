...
export default function Cart() {
  ...
  const totalSavings = cartItems.reduce(
    (sum, item) =>
      sum + (Number(item.discount_amount) || 0) * (Number(item.qty) || 1),
    0
  );
            ...
            {cartItems.map((item) => {
              ...
              return (
                <div key={item.id} className="py-4 flex gap-4">
                  <img src={item.image} alt={item.name} className="w-20 h-20 object-cover bg-gray-50 flex-shrink-0" />
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-sm font-semibold text-gray-800 block leading-tight">{item.name}</span>
                        <button onClick={() => removeFromCart(item.id)} className="text-gray-300 hover:text-pink-500 transition-colors p-1">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                          </svg>
                        </button>
                      ...
                      {hasDiscount && (
                        <span className="inline-block mt-1 text-[9px] font-bold uppercase tracking-wide text-pink-500 bg-pink-50 px-2 py-0.5 rounded-full">
                    ...
                    <div className="flex items-center justify-between mt-3 gap-2">
                      <div className="flex items-center border border-gray-200 rounded bg-white">
                        <button onClick={() => updateQty(item.id, item.qty - 1)} className="px-2.5 py-1 text-gray-500 hover:text-gray-900">−</button>
                        <span className="px-2.5 py-1 text-xs border-x border-gray-200 min-w-[32px] text-center font-medium">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, item.qty + 1)} className="px-2.5 py-1 text-gray-500 hover:text-gray-900">+</button>
                      </div>
                      <div className="text-right">
                        ...
                        {hasDiscount && (
                          <p className="text-[10px] text-gray-400 line-through">
                            ₹ {((Number(item.original_price) || 0) * (Number(item.qty) || 1)).toFixed(2)}
                          </p>
                        )}
                      </div>
                    </div>
